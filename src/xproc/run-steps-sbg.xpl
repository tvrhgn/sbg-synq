<p:declare-step xmlns:p="http://www.w3.org/ns/xproc"
	...
	<p:documentation>
	fase II: maak SBG-bestand op basis van de inhoud van de uitvoer van sbgem:epd-meting
	</p:documentation>
	...
	<p:variable name="sbg-patient.doc" select="concat( //target[1]/tmp, '/', 'sbg-patient-meting.xml')">
		<p:pipe step="sbg-steps" port="config" />
	</p:variable>
	
	<p:variable name="bm-info.doc" select="concat( //target[1]/tmp, '/', 'sbg-bmimport.xml')">
		<p:pipe step="sbg-steps" port="config" />
	</p:variable>
	...
	<!--  voeg timestamp in voor loggin-doeleinden -->	
	...
	<p:xquery name="sbg-patient-meting">
		...
		<p:input port="query">
			...
			return element { 'result' } { (), $za, 
				element { 'sbg-patient-meting' } { (), sbge:sbg-patient-meting( //patient-meting/*, $za//zorgdomein ) }
				}
			</c:query>
			</p:inline>
		</p:input>
	...
	<p:store name="store-sbg-patient-meting">
		<p:with-option name="href" select="concat( $tmp.dir, '/', $sbg-patient.doc )" />
	...
	<p:xquery name="sbg-benchmark-info">
		...
		<p:input port="query">
			...
			let $za := /context/result/zorgaanbieder  (: NB de logging za is nu ook ingevoegd :)
			return element { 'benchmark-info' } { (), /context/zorgaanbieder, sbgbm:build-sbg-bmimport( $za, //sbg-patient-meting//sbggz:Patient ) }
			</c:query>
			</p:inline>
		</p:input>
		<p:input port="parameters">
	...
	<p:viewport name="anonimiseer" match="//sbggz:Patient">
	 ...
	 <p:store name="store-benchmark-bestand">
		<p:with-option name="href" select="concat( $tmp.dir, '/', $bm-import.doc)" />
		<p:input port="source" select="/benchmark-info//*[local-name() eq 'BenchmarkImport']">
	...
	<p:store name="store-benchmark-info">
		<p:with-option name="href" select="concat( $tmp.dir, '/', $bm-info.doc)" />
