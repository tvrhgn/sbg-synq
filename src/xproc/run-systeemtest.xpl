...
<!--
standaard xerces past niet altijd in geheugen geeft een heap-fout :-( 

 <p:pipe step="sbg-validatie" port="report" />
	<p:try name="sbg-validatie">
		<p:group>
			<p:output port="result">
				<p:pipe step="sbg-schema" port="result" />
			</p:output>
			<p:output port="report" sequence="true">
				<p:empty />
			</p:output>

			<p:validate-with-xml-schema name="sbg-schema">
				<p:input port="source">
					<p:pipe step="sbg-systeemtest" port="source" />
				</p:input>
				<p:input port="schema">
					<p:pipe step="sbg-systeemtest" port="sbg-schema" />
				</p:input>
 ...
 -->
	...
	<p:validate-with-schematron assert-valid="false"
		name="schematron">
		<p:input port="source">
			<!--  <p:pipe step="sbg-validatie" port="result" />  -->
						<p:pipe step="sbg-systeemtest" port="source" />
		</p:input>
		<p:input port="schema">
	...
	<p:insert position="last-child" match="//*[@id='testinfo']"
		name="welcome-msg">
		<p:input port="insertion">
			<p:inline>
				<div class='test-info init-msg'>
					<h2>controle benchmark-bestand</h2>
					gebruik gekleurde velden als knoppen. (F5 = reset)
				</div>
			</p:inline>
		</p:input>
	</p:insert>
	<p:store name="store-checklist" method="html" version="4.0">
		<p:with-option name="href"
			select="concat( $report_dir, '/', $checklist_html)" />
	</p:store>
